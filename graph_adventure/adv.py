...
# 1. Check if the currentRoom is in the graph
     # if True:
        # move to next step
    # else:
    #   add currentRoomID to graph

# 2. Check the value of the exits
    # if there are values (have already visited room):
        # if there are ?:
            # go to next step
        # else:
            # we know all the values
    # else add all exit values (first time having visited room)
        # if previousRoomID exists, set the direction from whence you came
        # if value is unknown set the exit direction to "?"
# 3. Decide on direction to move
    #if no ? in graph:
        #break
    # if there are "?" in exits
        #if exit direction == ?, travel to that room
        #else go back to previous room (use traversalPath)
traversalPath = []
graph = {}
reverseReverse = {
    "n": "s",
...
def convert_to_direction(path, graph):
    needed_directions = []
    id_please = 1

    for room in path:
        for way_out in graph[room]:
            if graph[room][way_out] == path[id_please]:
                needed_directions.append(way_out)
                

                if path[id_please] == path[-1]:
                    return needed_directions

        id_please += 1

def bfs(starting, graph):
    q = Queue()
    visited = set()
    q.enqueue([starting])
    while q.size() > 0:
        ...
        for direction in graph[thing]:
            if graph[thing][direction] == "?":
                return convert_to_direction(path, graph)
            else:
                pass
        ...
        if thing not in visited:
            ...
            for room in graph[thing]:
                copied_path = path.copy()
                copied_path.append(graph[thing][room])
...
while searching:
    ...
    if not currentRoomID in graph:
        graph[currentRoomID] = {}
        found_exits = player.currentRoom.getExits()
        random.shuffle(found_exits)
        for exit in found_exits:
            graph[currentRoomID][exit] = "?"
    
    if previousRoomID is not None:
        graph[currentRoomID][reverseReverse[lastDirection]] = previousRoomID
    ...
    if traveling is not None:
        ...
        player.travel(traveling)
        traversalPath.append(traveling)
        graph[previousRoomID][traveling] = player.currentRoom.id
        lastDirection = traveling

    else:
        give_me_directions = bfs(currentRoomID, graph)
        previousRoomID = None

        if give_me_directions is None:
            searching = False
            break
        for exiter in give_me_directions:

            player.travel(exiter)
            traversalPath.append(exiter)
